...
import os
import sqlite3
sys.path.insert(0,'/workspace/builderschallenge/simple-file-upload/uploads/core/tensorflow/models/research/object_detection')
...
def model_form_upload(request):
    ...
    if request.method == 'POST':
        ...
        #print(str(filename))
        path = '/workspace/builderschallenge/simple-file-upload/media/documents/'
        if form.is_valid():
            form.save()
            documents = Document.objects.all()
            if len(documents) == 0:
...
def search(request):
    conn = sqlite3.connect('/workspace/builderschallenge/simple-file-upload/video.db')
    curs = conn.cursor()
